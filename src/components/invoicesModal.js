...
class InvoicesModal extends React.Component {
    ...
    this.props.form.validateFields(async (err, values) => {
      if (!err) {
        const mappedObject = {
          ...
          CompanyName: values.editedCompanyName ? values.editedCompanyName : values.CompanyName,
          FirstName: values.editedFirstName ? values.editedFirstName : values.FirstName,
          LastName: values.editedLastName ? values.editedLastName : values.LastName,
          Country: values.editedCountry ? values.editedCountry : values.Country,
          TotalPrice: values.editedTotalPrice ? values.editedTotalPrice : values.TotalPrice,
          TotalTax: values.editedTotalTax ? values.editedTotalTax : values.TotalTax,
          Revenue: values.editedRevenue ? values.editedRevenue : values.Revenue,
          CreatedDate: values.editedCreatedDate ? values.editedCreatedDate : values.CreatedDate,
          OrderItemName: values.editedItemName ? values.editedItemName : values.OrderItemName,
          Quantity: values.editedQuantity ? values.editedQuantity : values.Quantity,
          Units: values.editedUnits ? values.editedUnits : values.Units,
          Orders: values.editedOrders ? values.editedOrders : values.Orders,
          PromotionalCode: values.editedPromotionalCode ? values.editedPromotionalCode : values.PromotionalCode,
          RefBy: values.editedRefBy ? values.editedRefBy : values.RefBy,
          PayPalID: values.editedPayPalID ? values.editedPayPalID : values.PayPalID,
          SalesTypeId: values.editedSalesTypeId ? values.editedSalesTypeId : values.SalesTypeId,
          CurrencyID: values.editedCurrencyID ? values.editedCurrencyID : values.CurrencyID,
          CurrencyCode: values.editedCurrencyCode ? values.editedCurrencyCode : values.CurrencyCode,
        }
  ...
  render() {
    ...
    const { visible, isEdit, data } = this.props;
    ...
    return (
      ...
      >
        <Form layout="inline" labelCol={4} wrapperCol={14}>
          {!isEdit ?
            <>
            ...
            <>
              ...
              <Form.Item label="Revenue Date" hasFeedback validateStatus="success">
                {getFieldDecorator('editedCreatedDate', {
                  rules: [{ required: true, message: 'Please input your Revenue Date!' }],
                  initialValue: moment(new Date(data.record.CreatedDate).toLocaleDateString(), 'DD/MM/YYYY')
                })(
                  <DatePicker style={{ width: '100%' }} />
                )}
              </Form.Item>
              ...
              <Form.Item label="Promo Code">
                {getFieldDecorator('editedPromoCode', {
                  rules: [{ required: true, message: 'Please input your Promo Code!' }],
                  initialValue: data.record.PromotionalCode,
                })(
                  <Input
                    type="text"
                    placeholder="Promo Code"
